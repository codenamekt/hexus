...
def benchmark_embedder():
    ...
    print(
        f"Single embed (n={n}): mean={statistics.mean(times) * 1000:.1f}ms, "
    ...
    for batch_size in [1, 8, 32, 128]:
        ...
        print(
            f"Batch {batch_size:3d}: {elapsed * 1000:.1f}ms total, "
...
def benchmark_store(dsn: str):
    ...
    print(
        f"  Embedded {n_docs} docs in {embed_time:.2f}s ({n_docs / embed_time:.1f} docs/sec)"
    )
    ...
    print(
        f"  Inserted {n_docs} docs in {insert_time:.2f}s ({n_docs / insert_time:.1f} inserts/sec)"
    )
    ...
    for limit in [5, 10, 50]:
        ...
        for _ in range(20):
            t0 = time.perf_counter()
            rows_vec = store.search(
                query_embedding=query_vec, agent_identity=agent, limit=limit
            )
        ...
        for _ in range(20):
            t0 = time.perf_counter()
            rows_hybrid = store.hybrid_search(
        ...
        for _ in range(20):
            t0 = time.perf_counter()
            rows_rerank = store.hybrid_search(
                query_embedding=query_vec,
                query_text=query_text,
                agent_identity=agent,
                limit=limit,
        ...
        print(
            f"  Top-{limit:2d} Vector Search:  mean={statistics.mean(t_vec) * 1000:.2f}ms, p95={statistics.quantiles(t_vec, n=20)[18] * 1000:.2f}ms, results={len(rows_vec)}"
        )
        print(
            f"  Top-{limit:2d} Hybrid Search:  mean={statistics.mean(t_hybrid) * 1000:.2f}ms, p95={statistics.quantiles(t_hybrid, n=20)[18] * 1000:.2f}ms, results={len(rows_hybrid)}"
        )
        print(
            f"  Top-{limit:2d} Rerank Search:  mean={statistics.mean(t_rerank) * 1000:.2f}ms, p95={statistics.quantiles(t_rerank, n=20)[18] * 1000:.2f}ms, results={len(rows_rerank)}"
        )
        print("-" * 60)
